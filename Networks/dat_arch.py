    ...
    def __init__(self, dim, num_heads, reso=64, split_size=[2, 4], shift_size=[1, 2], expansion_factor=4.,
        ...
        if b_idx % 2 == 0:
            ...
            self.attn = Adaptive_Spatial_Attention(
                ...
                qk_scale=qk_scale, drop=drop, attn_drop=attn_drop, rg_idx=rg_idx, b_idx=b_idx
            )
        else:
...
class ResidualGroup(nn.Module):
    ...
    def __init__(self, dim, reso, num_heads, split_size=[2, 4], expansion_factor=4., qkv_bias=False, qk_scale=None,
                 drop=0., attn_drop=0., drop_paths=None, act_layer=nn.GELU, norm_layer=nn.LayerNorm, depth=2,
        ...
        self.blocks = nn.ModuleList([
            DATB(dim=dim, num_heads=num_heads,reso=reso,split_size=split_size,shift_size=[split_size[0] // 2, split_size[1] // 2],
            expansion_factor=expansion_factor,qkv_bias=qkv_bias,qk_scale=qk_scale,drop=drop,attn_drop=attn_drop,drop_path=drop_paths[i],act_layer=act_layer,
            norm_layer=norm_layer,rg_idx=rg_idx, b_idx=i,) for i in range(depth)])
    ...
    def __init__(self,
        ...
        for i in range(self.num_layers):
            layer = ResidualGroup(dim=embed_dim, num_heads=heads[i], reso=img_size,split_size=split_size,
                expansion_factor=expansion_factor,qkv_bias=qkv_bias,
                qk_scale=qk_scale,drop=drop_rate,attn_drop=attn_drop_rate,drop_paths=dpr[sum(depth[:i]):sum(depth[:i + 1])]
                ,act_layer=act_layer,
                norm_layer=norm_layer,depth=depth[i],use_chk=use_chk,resi_connection=resi_connection,rg_idx=i)
...
if __name__ == '__main__':
    ...
    height = 256
    width = 256
    model = DAT(upscale=2, in_chans=1, img_size=256, img_range=1.,depth=[6, 6, 6, 6, 6, 6], embed_dim=60,
        num_heads=[6, 6, 6, 6, 6, 6], expansion_factor=2, resi_connection='1conv',split_size=[8, 16],
    ).cuda().eval()
